export const calcDate = (date1, date2) => {
  ...
  const second = Math.round(date3 / 1000)
  return {
    leave1,
    leave2,
    leave3,
...
export function dateFormat(date, format) {
  ...
  formatData = formatData || 'yyyy-MM-dd hh:mm:ss';
  if (date !== 'Invalid Date') {
    ...
    if (/(y+)/.test(formatData)) formatData = formatData.replace(RegExp.$1,
      (date.getFullYear() + '').substr(4 - RegExp.$1.length));
    for (let k in o)
      if (new RegExp('(' + k + ')').test(formatData))
        formatData = formatData.replace(RegExp.$1,
