      ...
      optionScope: {
        ...
        column: [
          ...
          },
          {
            label: '规则类型',
            type: 'select',
            dicUrl: `/api/${projectConfig.URL_mallSystem_dict_dictionary}?code=data_scope_type`,
            props: {
              label: 'dictValue',
              value: 'dictKey',
            },
  ...
  methods: {
    initScope() {
      ...
      this.$refs.crudScope.option.column.filter(item => {
        const itemData = item
        if (watchModeData) {
          if (itemData.prop === 'scopeName') {
            this.formScope.scopeName = `${this.scopeMenuName} [${name}]`;
          }
          if (itemData.prop === 'resourceCode') {
            this.formScope.resourceCode = this.scopeMenuCode;
          }
          if (itemData.prop === 'scopeColumn') {
            this.formScope.scopeColumn = column;
          }
        }
        if (itemData.prop === 'scopeValue') {
          itemData.display = scopeType === 5;
        }
        return true
      });
    },
    ...
    handleDataScope(row) {
      ...
      this.scopeMenuCode = rDatas.code;
    ...
    rowDelScope(row) {
      ...
      this.$confirm('确定将选择数据删除?', {
        ...
        .then(() => {
          return removeDataScope(rDatas.id);
        })
        .then(() => {
          this.onLoadScope(this.pageScope);
          this.$message({
