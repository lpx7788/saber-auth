    ...
    <el-col :span="15">
      <basic-container>
        <el-button-group>
          ...
          <el-button v-if="permission.region_export"
                     type="primary"
                     size="small"
...
export default {
      ...
      regionOption: {
        ...
        column: [
          ...
          },
          {
            ...
            prop: 'regionLevel',
            type: 'radio',
            dicUrl: `/api/${projectConfig.URL_mallSystem_dict_dictionary}?code=region`,
            props: {
              label: 'dictValue',
              value: 'dictKey',
            },
        ...
        column: [
          {
            ...
            action: `/api/${projectConfig.URL_mallSystem_region_import_region}`,
          },
          {
            label: '数据覆盖',
            prop: 'isCovered',
        ...
        column: [
          {
            ...
            dicUrl: `/api/${projectConfig.URL_mallSystem_region_select}`,
            span: 24,
          },
          {
            label: '地市',
            ...
            dicFlag: false,
            dicUrl: `/api/${projectConfig.URL_mallSystem_region_select}?code={{key}}`,
            span: 24,
          },
          {
            label: '区县',
  ...
  watch: {
    'regionForm.subCode': function () {
      this.regionForm.code = this.regionForm.parentCode + this.regionForm.subCode;
    },
    'excelForm.isCovered': function () {
      if (this.excelForm.isCovered !== '') {
        const column = this.findObject(this.excelOption.column, 'excelFile');
        column.action = `/api/${projectConfig.URL_mallSystem_region_import_region}?isCovered=${this.excelForm.isCovered}`;
      }
    },
  },
  computed: {
  ...
  methods: {
    ...
    addChildren() {
      ...
      this.regionForm.regionLevel =
        (this.regionForm.regionLevel === 5)
          ? 5
          : this.regionForm.regionLevel + 1;
    },
    handleSubmit(form, done, loading) {
      const formData = form
      const parentCode = formData.parentCode === this.topCode ? '' : formData.parentCode;
      formData.code = parentCode + formData.subCode;
    ...
    handleExport() {
      this.$confirm('是否导出行政区划数据?', '提示', {
        ...
        window.open(`/api/${projectConfig.URL_mallSystem_region_export_region}?${this.website.tokenHeader}=${getToken()}`);
      });
    },
    handleTemplate() {
      window.open(`/api/${projectConfig.URL_mallSystem_region_export_template}?${this.website.tokenHeader}=${getToken()}`);
    },
  },
};
</script>
