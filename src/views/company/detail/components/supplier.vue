    ...
    <avue-crud :option="option"
      ...
      <template slot-scope="scope"
        ...
        <el-popover placement="right"
          ...
          <p v-for="(item,idx) in scope.row.purchaseList"
             :key="idx">{{item.contractName}}：{{item.availableQuotaNum}}</p>
          <el-button type="text"
                     slot="reference">
            {{scope.row.purchaseList[0].contractName}}：
      ...
      <template slot-scope="scope"
        ...
        <el-popover placement="right"
          ...
          <el-button type="text"
                     slot="reference">
            {{scope.row.salesList[0].contractName}}：{{scope.row.salesList[0].availableQuotaNum}}
          </el-button>
        </el-popover>
      </template>
    </avue-crud>
...
<script>
import { queryCompanySuppliers } from '@/api/company/index'
