...
export default {
        ...
        column: [
          {
            ...
            prop: 'flowCategory',
            type: 'select',
            dicUrl: `/api/${projectConfig.URL_mallSystem_dict_dictionary}?code=flow`,
            props: {
              label: 'dictValue',
              value: 'dictKey',
            },
          ...
          },
          {
            ...
            action: `/api/${projectConfig.URL_mallFlow_manager_checkUpload}`,
          },
        ],
      },
    }
  ...
  watch: {
    'form.flowMode': function () {
      this.$refs.form.option.column.filter(item => {
        const itemData = item
        if (itemData.prop === 'tenantId') {
          itemData.display = this.form.flowMode === 2;
          return itemData.display
        }
        return true
      });
    },
