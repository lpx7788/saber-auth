        ...
        column: [
          {
            ...
            searchLabelWidth: 50,
            row: true,
            dicUrl: `/api/${projectConfig.URL_mallSystem_dict_dictionary}?code=oss`,
            props: {
              label: 'dictValue',
              value: 'dictKey',
            },
  ...
  watch: {
    'form.category': function () {
      ...
      this.$refs.crud.option.column.filter(item => {
        const itenData = item
        if (itenData.prop === 'appId') {
          itenData.display = category === 4;
        }
        if (itenData.prop === 'region') {
          itenData.display = category === 4;
        }
        return true
      });
    },
    'debugForm.code': function () {
      const column = this.findObject(this.debugOption.column, 'backgroundUrl');
      column.action = `/api/${projectConfig.URL_mallResource_oss_endpoint_putFile}?code=${this.debugForm.code}`;
    },
  },
  computed: {
