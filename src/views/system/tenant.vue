    ...
    <avue-crud :option="option"
      ...
      <template slot="menuLeft">
        ...
        <el-button size="small"
                   plain
                   v-if="userInfo.roleName.includes('administrator')"
...
<script>
import { add, datasource, getDetail, getList, remove, setting, update } from '@/api/system/tenant';
...
export default {
      ...
      option: {
        ...
        column: [
          ...
          },
          {
            ...
            action: `/api/${projectConfig.URL_mallResource_oss_endpoint_putFile}`,
            propsHttp: {
              res: 'data',
              url: 'link',
            },
        ...
        column: [
          {
            ...
            span: 24,
            type: 'select',
            dicUrl: `/api/${projectConfig.URL_mallDevelop_datasource_select}`,
            props: {
              label: 'name',
              value: 'id',
            },
  ...
  methods: {
    ...
    beforeOpen(done, type) {
      if (['view'].includes(type)) {
        getDetail(this.form.id).then(res => {
          ...
          if (!(returnData.accountNumber > 0)) {
            returnData.accountNumber = '不限制';
          }
          if (!returnData.expireTime) {
            returnData.expireTime = '不限制';
          }
    ...
    handleSetting() {
      ...
      if (this.selectionList.length === 1) {
        getDetail(this.selectionList[0].id).then(res => {
          const returnData = res.data.data;
          this.settingForm.accountNumber = returnData.accountNumber;
          this.settingForm.expireTime = returnData.expireTime;
        });
      } else {
    ...
    handleDatasource() {
      ...
      getDetail(this.selectionList[0].id).then(res => {
        const returnData = res.data.data;
        this.datasourceForm.datasourceId = returnData.datasourceId;
      });
