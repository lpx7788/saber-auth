      ...
      <el-row type="flex"
        ...
        <el-form-item>
          ...
          <el-button type="danger"
                     @click="handleDisagree">驳回</el-button>
...
export default {
  ...
  methods: {
    init() {
      ...
      this.src = `/api/${projectConfig.URL_mallFlow_process_diagramView}?processInstanceId=${this.$route.params.processInstanceId}`;
      historyFlowList(this.processInstanceId).then(res => {
        const returnData = res.data;
        if (returnData.success) {
          this.flowList = returnData.data;
        }
      }).catch(() => { })
      leaveDetail(this.businessId).then(res => {
        const returnData = res.data;
        if (returnData.success) {
          this.form = returnData.data;
        }
      }).catch(() => { })
    },
    handleAgree() {
      ...
      completeTask(params).then(res => {
        const returnData = res.data;
        if (returnData.success) {
          this.$message.success(returnData.msg);
          this.$router.$avueRouter.closeTag();
          this.$router.push({ path: '/work/start' });
        } else {
          this.$message.error(returnData.msg || '提交失败');
        }
      }).catch(() => { })
    },
    handleDisagree() {
